...
/** Parse YYYY-MM-DD en fecha local (evita desfase UTC). */
...
/** Suma meses a una fecha YYYY-MM-DD y devuelve YYYY-MM-DD en calendario local. */
function addMonthsFromYMD(yyyyMmDd, months) {
  const base = parseFechaYYYYMMDD(yyyyMmDd);
  if (!base) return null;
  const v = new Date(base.getFullYear(), base.getMonth(), base.getDate());
  v.setMonth(v.getMonth() + months);
  return formatoYMDLocal(v);
}
...
function Enrolar_proceso() {
  ...
  const [fechaEnrolamiento, setFechaEnrolamiento] = useState(""); // YYYY-MM-DD — elegida por el usuario
    ...
    const fetchUsuarioSesion = async () => {
      try {
        ...
        if (res.data.success && res.data.data) {
          ...
          const nombre =
            emp.emp_nombre || emp.NOMBRE || emp.nombre || String(empId);
  ...
  const handleFileMasivo = async (e) => {
    ...
    if (
      ...
      showAlert(
        "Formato no válido. Use Excel (.xlsx, .xls) o CSV (.csv)",
        "error",
      );
      return;
    }
        ...
        try {
          ...
          try {
            const res2 = await axios.get(
              `/api/empleados_matriz?emp_id=${encodeURIComponent(empId)}`,
            );
      ...
      if (resultados.length > 0) {
        ...
        showAlert(
          noEncontrados.length > 0
            ? "Ningún empleado del archivo fue encontrado (lista descargada)"
  ...
  const handleGuardar = async () => {
    ...
    if (modo === "empleado") {
      ...
      if (!fechaEnrolamiento || !parseFechaYYYYMMDD(fechaEnrolamiento)) {
        showAlert("Seleccione la fecha de enrolamiento.", "error");
        return;
      }
      ...
      enrolamientos = procesosSeleccionados.map((proceso) => {
        ...
        const vencStr = esCertificable
          ? addMonthsFromYMD(fechaEnrolamiento, 6)
          : null;
        return {
          emp_id: empId,
          emp_nombre: empNombre,
      ...
      if (listaEmpleados.length === 0) {
        showAlert(
          masivo
            ? "Por favor carga un archivo Excel con empleados"
            : "Por favor agrega al menos un empleado",
          "error",
        );
        return;
      }
      ...
      const getEmpId = (emp) =>
        masivo
          ? emp.emp_id
          : emp.NUM_EMPLEADO || emp.num_empleado || emp.emp_id || emp.id;
      const verificaciones = await Promise.all(
      ...
      if (masivo) {
        enrolamientos = empleadosMasivos.map((emp) => {
          ...
          return {
            ...
            enrolado_por: enroladoPor || "Sistema",
            es_certificacion: esCertificable,
            fecha: emp.fecha || fechaBase.toISOString().split("T")[0],
    ...
    <Box sx={{ p: 3 }}>
      ...
      >
        <CardContent sx={{ p: 3 }}>
          ...
          {modo === "proceso" && (
            <Box>
                ...
                >
                  {procesos.map((proceso) => {
                    const esCertificable =
                      ...
                      proceso.certificable.toString().toLowerCase() === "si";
                    return (
                      <MenuItem key={proceso.id} value={proceso.id.toString()}>
                        {proceso.nombre}
                        {esCertificable ? " (Certificable)" : ""}
                      </MenuItem>
                    );
              ...
              {procesoSeleccionado && (
                <>
                  <Box
                    sx={{ mb: 2, mt: 3, display: "flex", alignItems: "center" }}
                  >
                    <FormControlLabel
                      control={
                        <Checkbox
                  ...
                  {masivo ? (
                    <>
                      <Typography variant="h6" gutterBottom sx={{ mb: 2 }}>
                        Paso 2: Carga archivo Excel (col. 1 = Nº empleado, col.
                        2 = Fecha)
                      </Typography>
                      <Box
                        display="flex"
                      ...
                      >
                        ...
                        <Typography variant="body2" color="text.secondary">
                          Primera columna: número de empleado. Segunda: fecha
                          (se suma 6 meses para vencimiento).
                        </Typography>
                      </Box>
                        ...
                        <Paper sx={{ p: 2, mb: 3 }}>
                          ...
                          <List sx={{ maxHeight: 200, overflow: "auto" }}>
                            ...
                            {empleadosMasivos.length > 30 && (
                              <ListItem>
                                <ListItemText
                                  secondary={`... y ${empleadosMasivos.length - 30} más`}
                                />
                              </ListItem>
                            )}
                          </List>
          ...
          {(modo === "empleado" || (modo === "proceso" && !masivo)) && (
            <Box sx={{ mt: 3, mb: 1 }}>
              <TextField
                type="date"
                label="Fecha de enrolamiento"
                value={fechaEnrolamiento}
                onChange={(e) => setFechaEnrolamiento(e.target.value)}
                InputLabelProps={{ shrink: true }}
                sx={{ maxWidth: 280 }}
                helperText="Fecha en que se registra el enrolamiento. En procesos certificables, la vigencia (+6 meses) se calcula desde esta fecha."
              />
            </Box>
          )}
