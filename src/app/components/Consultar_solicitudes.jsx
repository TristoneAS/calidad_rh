...
/** Extrae YYYY-MM-DD de un valor de fecha (API / MySQL). */
function toYMD(s) {
...
/** Suma 6 meses a una fecha YYYY-MM-DD en calendario local. */
function vencimientoMasSeisMesesDesde(ymd) {
  if (!ymd) return null;
  ...
  const v = new Date(y, mo, d);
  if (v.getFullYear() !== y || v.getMonth() !== mo || v.getDate() !== d)
    return null;
  v.setMonth(v.getMonth() + 6);
...
function Consultar_solicitudes({ modo = "full", initialTab = 0 }) {
  ...
  const asignarCertificacion = async (solicitud) => {
    ...
    try {
      ...
      const prevYmd = yaExiste
        ? toYMD(enrolamientoExistente?.fecha_vencimiento)
        : null;
      if (prevYmd) {
        fechaVencimiento = vencimientoMasSeisMesesDesde(prevYmd);
      }
      if (!fechaVencimiento) {
        const hoy = new Date();
      ...
      try {
        ...
        if (
          ...
          Array.isArray(resHist.data.data) &&
          resHist.data.data.length > 0
        ) {
          esRenovacion = true;
      ...
      // Renovación: la certificación "empieza" el día en que venció la anterior, no hoy
      if (yaExiste && prevYmd) {
        historialPayload.fecha_certificacion = prevYmd;
      }
      await axios.post("/api/certificaciones", historialPayload);
  ...
  const columns = [
    ...
    },
    {
      ...
      renderHeader: () => (
        <Box
          sx={{ whiteSpace: "pre-line", textAlign: "center", lineHeight: 1.3 }}
        >
          ID Proceso
          {"\n"}
          Certificable
