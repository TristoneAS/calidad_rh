...
// Fondos opacos para encabezados sticky (evita que se vean las filas debajo)
const headerBgCurso = "#DBEAFE";
const headerBgCursoHover = "#BFDBFE";
const headerBgProceso = "#DBEAFE";
const headerBgProcesoCert = "#D1FAE5";
const headerBgProcesoHover = "#BFDBFE";
const headerBgProcesoCertHover = "#A7F3D0";
...
function Consultar_matriz_habilidades() {
  ...
  const etiquetaTipoHistorial = (tipo) => {
    const map = {
      nueva: "Nueva",
      renovacion: "Renovación",
      eliminacion: "Eliminación",
    };
    return map[tipo] || tipo || "Nueva";
  };

  const procesoEsCertificable = (proceso) =>
  ...
  const obtenerNombreUsuario = () => {
    ...
    try {
      ...
      if (storedUser) {
        ...
        if (Array.isArray(users) && users[0]?.displayName) {
          return users[0].displayName;
        }
        if (data?.emp_nombre) return data.emp_nombre;
      }
      return window.localStorage.getItem("usuario") || "Sistema";
    } catch {
      return "Sistema";
    }
  };

  const abrirConfirmarEliminar = (empId, proceso, empleadoNombre) => {
    setEliminarDialog({
      open: true,
      empId,
  ...
  const cerrarEliminarDialog = () => {
    setEliminarDialog((p) => ({ ...p, open: false, loading: false }));
  };

  const confirmarEliminarCertificacion = async () => {
    const { empId, idProceso } = eliminarDialog;
    if (!empId || !idProceso) return;
    try {
      setEliminarDialog((p) => ({ ...p, loading: true }));
      const res = await axios.delete("/api/certificaciones", {
        data: {
          emp_id: empId,
          id_proceso: idProceso,
          eliminado_por: obtenerNombreUsuario(),
        },
      });
      if (!res.data?.success) {
        showAlert(res.data?.error || "Error al eliminar la certificación", "error");
        setEliminarDialog((p) => ({ ...p, loading: false }));
        return;
      }
      setEmpleadosProcesos((prev) =>
        prev.filter(
          (ep) =>
            !(
              ep.emp_id?.toString() === String(empId) &&
              ep.id_proceso?.toString() === String(idProceso)
            ),
        ),
      );
      cerrarEliminarDialog();
      showAlert("Certificación eliminada y registrada en historial", "success");
    } catch (err) {
      console.error("Error al eliminar certificación:", err);
      showAlert(
        err.response?.data?.error || "Error al eliminar la certificación",
        "error",
      );
      setEliminarDialog((p) => ({ ...p, loading: false }));
    }
    ...
    <Box sx={{ p: 3 }}>
                          ...
                          return (
                            <TableCell
                              ...
                              sx={{
                                ...
                                "&:hover": {
                                  bgcolor: esCertificable
                                    ? headerBgProcesoCertHover
                                    : headerBgProcesoHover,
                                },
                              }}
                            >
                              <Box
      ...
      >
        ...
        <DialogContent>
          {historialDialog.loading ? (
            ...
            <Typography color="text.secondary">
              Sin registros de certificación
            </Typography>
          ) : (
            <TableContainer>
              <Table size="small">
                ...
                <TableBody>
                  {historialDialog.registros.map((r) => (
                    <TableRow key={r.id}>
                      <TableCell>
                        {formatearFecha(r.fecha_certificacion)}
                      </TableCell>
                      <TableCell>
                        {formatearFecha(r.fecha_vencimiento)}
                      </TableCell>
                      <TableCell>{etiquetaTipoHistorial(r.tipo)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
        ...
        <DialogActions sx={{ px: 3, pb: 2, pt: 0 }}>
          <Button variant="contained" onClick={cerrarHistorialModal} fullWidth>
            Cerrar
          </Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog
        ...
        onClose={(event, reason) => {
          if (reason === "backdropClick" || eliminarDialog.loading) return;
          cerrarEliminarDialog();
        }}
        maxWidth="xs"
        fullWidth
      ...
      >
        <DialogTitle>Eliminar certificación</DialogTitle>
        <DialogContent>
          <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
            {eliminarDialog.nombreEmpleado} • {eliminarDialog.nombreProceso}
          </Typography>
          <Typography>
            ¿Confirma eliminar la certificación activa? La acción quedará
            registrada en el historial y el empleado dejará de aparecer como
            certificado en la matriz.
          </Typography>
        </DialogContent>
        <DialogActions sx={{ px: 3, pb: 2 }}>
          <Button
            onClick={cerrarEliminarDialog}
            disabled={eliminarDialog.loading}
          >
            Cancelar
          </Button>
          <Button
            variant="contained"
            color="error"
            onClick={confirmarEliminarCertificacion}
            disabled={eliminarDialog.loading}
            startIcon={
              eliminarDialog.loading ? (
                <CircularProgress size={18} color="inherit" />
              ) : null
            }
          >
            {eliminarDialog.loading ? "Eliminando…" : "Eliminar"}
          </Button>
        </DialogActions>
      </Dialog>
    </Box>
