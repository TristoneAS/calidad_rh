...
function Alta_curso() {
  ...
  const [editDialog, setEditDialog] = useState({ open: false, curso: null });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        showAlert(response.data.error || "Error al crear el curso", "error");
      }
    } catch (error) {
      console.error("Error al crear curso:", error);
      showAlert(
        error.response?.data?.error || "Error al crear el curso",
        "error"
      );
    } finally {
      setLoading(false);
  ...
  const handleEdit = (curso) => {
    setEditDialog({ open: true, curso: { ...curso } });
  };
  ...
  const handleUpdate = async () => {
    const { curso } = editDialog;
    if (!curso.nombre.trim() || !curso.descripcion.trim()) {
      showAlert("Por favor completa todos los campos", "error");
      return;
    }
    ...
    try {
      setLoading(true);
      const response = await axios.put("/api/cursos", {
        id: curso.id,
      ...
      if (response.data.success) {
        ...
        setEditDialog({ open: false, curso: null });
        fetchCursos();
      } else {
        showAlert(
          response.data.error || "Error al actualizar el curso",
          "error"
        );
      }
    } catch (error) {
  ...
  const handleDeleteConfirm = async () => {
    try {
      setLoading(true);
      const response = await axios.delete(`/api/cursos?id=${deleteDialog.id}`);
      ...
      if (response.data.success) {
        ...
        showAlert(response.data.error || "Error al eliminar el curso", "error");
      }
    } catch (error) {
      console.error("Error al eliminar curso:", error);
      showAlert(
        error.response?.data?.error || "Error al eliminar el curso",
        "error"
      );
    } finally {
      setLoading(false);
